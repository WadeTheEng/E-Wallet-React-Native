...
export default class PayConfirmView extends Component {
  ...
  render() {
    ...
    const dateTime = Moment(payData.datetime).format("DD/MM/YYYY a hh:mm:ss");
        ...
        <ScrollView style={{ flex: 1, marginLeft: 20, marginRight: 20 }}>
          ...
          >
            <Text style={styles.txtBottomDesc}>
              {`${payData.converted_amount} ${payData.devise_iso_code}`}
            </Text>
            <KralissMoneyIconCell
              style={{ position: "absolute", right: 0 }}
              title={`${i18n.t("sendMoney.thats")} ${payData.kraliss_amount}`}
          ...
          <KralissClearCell
            ...
            detailContent={payData.receiver_name}
          />
          <KralissClearCell
            style={styles.marginCell}
          ...
          <Text style={styles.txtBottomDesc}>{payData.transaction_number}</Text>
